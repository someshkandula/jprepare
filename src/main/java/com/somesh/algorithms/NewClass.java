...
public class NewClass {
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> chunks = breakIntoChunks(intList, 3);
    ...
    private static List<List<Integer>> breakIntoChunks(List<Integer> intList,int size) {
        List<List<Integer>> chunks = new ArrayList<>();
        int curr =0;
        for(int i = 0; i<intList.size()-1; i++){
            if(intList.get(i).compareTo(intList.get(i+1))>0 || i==intList.size()-1){
                chunks.add(intList.subList(curr, i+size));
                curr = i+1;
            }
         }
